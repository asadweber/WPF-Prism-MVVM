...
namespace DAC.Core.DataAccessObjects.General
{
	...
	internal sealed partial class tran_psychoexamresultDataAccessObjects : BaseDataAccess, Itran_psychoexamresultDataAccessObjects
	{
        ...
        public async Task<long> AddPsychoExamResult(Database db,
        DbTransaction transaction, tran_psychoexamresultEntity tran_psychoexamresult, CancellationToken cancellationToken)
        {
            long returnCode = -99;
            ...
            using (DbCommand cmd = db.GetStoredProcCommand(SP))
            {
                ...
                {
                    throw GetDataAccessException(ex, SourceOfException("Itran_psychoexamresultDataAccess.AddPsychoExamResult"));
                }
                cmd.Dispose();
            }
            return returnCode;
        ...
        public async Task<tran_psychoexamresultEntity> GetLatestPsychoExamResult(tran_psychoexamresultEntity tran_psychoexamresult, CancellationToken cancellationToken)
        {
            try
            {
                const string SP = "KAF_GetLatestPsychoExamResult_ByBasicInfoIDRegistrationID";
