...
namespace DAC.Core.DataAccessObjects.General
{
	...
	internal sealed partial class owin_forminfoDataAccessObjects : BaseDataAccess, Iowin_forminfoDataAccessObjects
	{
       ...
       CancellationToken cancellationToken) 
       {
        ...
        async Task<long> Iowin_forminfoDataAccessObjects.SaveMasterDetcnf_studentmenuskip(owin_forminfoEntity masterEntity, 
            ...
            try
            {
                using (DbCommand cmd = Database.GetStoredProcCommand(SP))
				{
                    ...
                    if (returnCode>0)
                    {
                        ...
                        cnf_studentmenuskipDataAccessObjects objcnf_studentmenuskip=new cnf_studentmenuskipDataAccessObjects(this.Context);
                        objcnf_studentmenuskip.SaveList(Database, transaction, listAdded, listUpdated, listDeleted, cancellationToken);
                    }
                    if (masterEntity.CurrentState == BaseEntity.EntityState.Deleted)
        ...
        async Task<long> Iowin_forminfoDataAccessObjects.SaveMasterDetcnf_studentmenuskipdetail(owin_forminfoEntity masterEntity, 
        ...
        #region for Dropdown 
		async Task<IList<gen_dropdownEntity>> Iowin_forminfoDataAccessObjects.GetDataForDropDown(owin_forminfoEntity owin_forminfo, CancellationToken cancellationToken)
		{
			try
			{
				const string SP = "owin_forminfo_GAPgDropDown";
				using (DbCommand cmd = Database.GetStoredProcCommand(SP))
				{
					AddTotalRecordParameter(cmd);
					AddSortExpressionParameter(cmd, owin_forminfo.SortExpression);
			...
			{
				throw GetDataAccessException(ex, SourceOfException("Iowin_forminfoDataAccess.GetDataForDropDown"));
			}
		}
		#endregion
