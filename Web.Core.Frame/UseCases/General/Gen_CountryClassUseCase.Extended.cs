...
namespace Web.Core.Frame.UseCases
{
    ...
    {
        public async Task<bool> GetCountryClassDropDown(Gen_CountryClassRequest message, IDDLRequestHandler<Gen_CountryClassResponse> outputPort)
        {
            ...
            try
            {
                ...
                .GetCountryClassDropDown(message.Objgen_countryclass, cancellationToken);

                if (oblist != null && oblist.Count > 0)
                {
                    var select = (from data in oblist
                                  where data.countryid == 1
                                  select new { Id = data.classid, Text = data.classname }).ToList();

                    outputPort.GetDropDown(new Gen_CountryClassResponse(new AjaxResponse(oblist[0].RETURN_KEY, select), true, null));
