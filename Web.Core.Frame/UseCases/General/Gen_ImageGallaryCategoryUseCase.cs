...
{
    public sealed partial class Gen_ImageGallaryCategoryUseCase : IGen_ImageGallaryCategoryUseCase
    {
        ...
        public async Task<bool> GetAll(Gen_ImageGallaryCategoryRequest message, ICRUDRequestHandler<Gen_ImageGallaryCategoryResponse> outputPort)
        {
            ...
            try
            {
                ...
                .GetAll(message.Objgen_imagegallarycategory, cancellationToken);
                ...
                if (oblist != null && oblist.Count > 0)
                {
                    ...
                    var data = (from t in oblist
                                select new
                                {
									 ...
									 t.imagegallarycategory,
									 t.descriptions,                                    datatablebuttonscode = objDTBtnPanel.genDTBtnPanel(message.Objgen_imagegallarycategory.ControllerName, t.imagegallarycategoryid, "imagegallarycategoryid", _contextAccessor.HttpContext.User.Identity as ClaimsIdentity, btnActionList)
                                }).ToList();

                    outputPort.GetListView(new Gen_ImageGallaryCategoryResponse(new AjaxResponse(oblist[0].RETURN_KEY, data), true, null));
                    return true;
                }
                else
                {
        ...
        public async Task<bool> Save(Gen_ImageGallaryCategoryRequest message, ICRUDRequestHandler<Gen_ImageGallaryCategoryResponse> outputPort)
        {
            ...
            try
            {
                ...
                outputPort.Save(new Gen_ImageGallaryCategoryResponse(new AjaxResponse("200", _sharedLocalizer["DATA_SAVE_CONFIRMATION"].Value, CLL.LLClasses._Status._statusSuccess, CLL.LLClasses._Status._titleInformation, "/"
                ), true, null));
                return true;
            }
            catch (Exception ex)
        ...
        public async Task<bool> Update(Gen_ImageGallaryCategoryRequest message, ICRUDRequestHandler<Gen_ImageGallaryCategoryResponse> outputPort)
        {
            ...
            try
            {
                ...
                outputPort.Update(new Gen_ImageGallaryCategoryResponse(new AjaxResponse("200", _sharedLocalizer["DATA_UPDATE_CONFIRMATION"].Value, CLL.LLClasses._Status._statusSuccess, CLL.LLClasses._Status._titleInformation, "/"
                        ), true, null));
                return true;
            }
            catch (Exception ex)
        ...
        public async Task<bool> Delete(Gen_ImageGallaryCategoryRequest message, ICRUDRequestHandler<Gen_ImageGallaryCategoryResponse> outputPort)
        {
            ...
            try
            {
                ...
                outputPort.Delete(new Gen_ImageGallaryCategoryResponse(new AjaxResponse("200", _sharedLocalizer["DATA_DELETE_CONFIRMATION"].Value, CLL.LLClasses._Status._statusSuccess, CLL.LLClasses._Status._titleInformation, "/"
                       ), true, null));
                return true;
            }
            catch (Exception ex)
        ...
        /// <summary>
		/// GetDataForDropDown Gen_ImageGallaryCategory
		/// </summary>
		/// <param name="response"></param>
		public async Task<bool> GetDataForDropDown(Gen_ImageGallaryCategoryRequest message, IDDLRequestHandler<Gen_ImageGallaryCategoryResponse> outputPort)
		{
			CancellationToken cancellationToken = new CancellationToken();
			try
			{
				IList<gen_dropdownEntity> oblist = await BFC.Core.FacadeCreatorObjects.General.gen_imagegallarycategoryFCC.GetFacadeCreate(_contextAccessor).GetDataForDropDown(message.Objgen_imagegallarycategory, cancellationToken);
				if (oblist != null && oblist.Count > 0)
				{
					outputPort.GetDropDown(new Gen_ImageGallaryCategoryResponse(new AjaxResponse(oblist[0].RETURN_KEY, oblist), true, null));
					return true;
				}
				else
				{
